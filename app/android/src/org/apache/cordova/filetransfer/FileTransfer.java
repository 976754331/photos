...
public class FileTransfer extends CordovaPlugin {
	...
	private static class SimpleTrackingInputStream extends TrackingInputStream {
		...
		// Note: FilterInputStream delegates read(byte[] bytes) to the below
		// method,
	...
	private static void addHeadersToRequest(URLConnection connection, JSONObject headers) {
		try {
			for (Iterator<?> iter = headers.keys(); iter.hasNext();) {
				 ...
				 * Additionally, spaces are not allowed in header names. RFC
				 * 2046 Quoted-printable encoding may be used to encode
				 * arbitrary characters, but we donon- not do that encoding
				 * here.
				 */
	 ...
	 *            fileName File name to be used on server args[4] mimeType
	 *            Describes file content type args[5] params key:value pairs of
		...
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				...
				try {
					...
					// if we specified a Content-Type header, don't do multipart
					// form upload
					 ...
					 * string, so that we can add it to the contentSize, since
					 * it is part of the body of the HTTP request.
					 */
					...
					try {
						inStream = getInputStream(conn,inputStream);
						synchronized (context) {
							if (context.aborted) {
								return;
							}
	...
	private static TrackingInputStream getInputStream(URLConnection conn,InputStream inputStream) throws IOException {
		String encoding = conn.getContentEncoding();
		TrackingGZIPInputStream gzipInputStream;
		SimpleTrackingInputStream simpleTrackingInputStream;
//		InputStream inputStream = null;
		if (encoding != null && encoding.equalsIgnoreCase("gzip")) {
			inputStream = conn.getInputStream();
			gzipInputStream = new TrackingGZIPInputStream(new ExposedGZIPInputStream(inputStream));
			// if (inputStream != null) {
			// inputStream.close();
			// }
		...
		inputStream = conn.getInputStream();
		simpleTrackingInputStream = new SimpleTrackingInputStream(inputStream);
		// if (inputStream != null) {
		// inputStream.close();
		// }
	...
	private static final HostnameVerifier DO_NOT_VERIFY = org.apache.http.conn.ssl.SSLSocketFactory.STRICT_HOSTNAME_VERIFIER;
	// Create a trust manager that does not validate certificate chains
	private static final TrustManager[] trustAllCerts = new TrustManager[] { new fileTransFerX509TrustManager() };
		...
		if (connection != null) {
			...
			try {
				if (connection instanceof HttpURLConnection) {
					...
					err = ((HttpURLConnection) connection).getErrorStream();
					inputStreamReader = new InputStreamReader(err, "UTF-8");
					if (err != null) {
				...
				// IOException can leave connection object in a bad state, so
	...
	private void download(final String source, final String target, JSONArray args, CallbackContext callbackContext)
		 ...
		 * Cordova. Previously the CordovaWebView class had a method,
		 * getWhitelist, which would return a Whitelist object. Since the fixed
		 * whitelist is removed in Cordova 4.x, the correct call now is to
		 * shouldAllowRequest from the plugin manager.
		 */
				...
				try {
					...
					if (isLocalTransfer) {
						...
						if (connection.getResponseCode() == HttpURLConnection.HTTP_NOT_MODIFIED) {
							...
							if (connection.getContentEncoding() == null
								...
								// Only trust content-length header if we
								// understand
							...
							inputStream = getInputStream(connection,inputStream2);
						}
					}
					...
					if (!cached) {
						try {
							...
							while (buffer != null && inputStream != null
...
class fileTransFerX509TrustManager implements X509TrustManager {
